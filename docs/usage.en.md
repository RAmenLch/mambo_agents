...
| Mode | Function | Characteristic |
|------|----------|----------------|
| **Fine-grained** | `create_mambo_agent()` | Full control over every parameter |
| **Plug-and-play** | `create_powerful_agent()` | Sensible defaults, one-line fully-featured agent |
...
agent = create_powerful_agent(
    "gpt-4o",
    workspace="/tmp/myproject"  # specify a local working directory
)
...
When `interrupt_on` is set, `create_powerful_agent()` automatically enables **AI security pre-review** — tool calls are evaluated by the main model first; only those flagged as high-risk escalate to human approval.
...
### 4.2 `create_powerful_agent()` — Plug-and-Play Mode
...
|---------|---------------|
| Conversation Summarization | triggers at 200k tokens, keeps last 20 messages, backend persistence |
| Task Planning | `write_plans` tool available |
| General-purpose Sub-agent | a backend sub-agent sharing the main agent's tools |
| AI Security Review | auto-enabled when `interrupt_on` is set (reuses the main model) |
...
3. [MamboSummarizationMiddleware]   ← conversation summarization (when summarization is not None)
4. [user-defined middleware]        ← passed via middleware parameter
5. [SubAgentMiddleware]             ← synchronous sub-agents
6. [AsyncSubAgentMiddleware]        ← async sub-agents
7. [AutoSecurityReviewMiddleware | HumanInTheLoopMiddleware]  ← security review
8. PatchToolCallsMiddleware         ← fix dangling tool calls (always enabled)
9. ReorderToolMessagesMiddleware    ← reorder tool messages (always enabled)
```
...
# Or via create_powerful_agent (enabled by default)
