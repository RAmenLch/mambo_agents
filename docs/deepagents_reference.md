...
| **Cross-session Persistence** | ✅ `StoreBackend` (backed by LangGraph `BaseStore`) | ✅ `StoreBackend` (backed by LangGraph `BaseStore`, `thread_id` resolved at runtime from graph config) | Mambo's StoreBackend is self-implemented, inherits from `BackendProtocol`, resolves `thread_id` from the graph config at runtime (locked only when explicitly passed) |
...
| In-memory Storage | `StateBackend` | `StoreBackend` (self-implemented, backed by LangGraph `BaseStore`, `thread_id` resolved at runtime from graph config) |
...
| Conversation Summarization | `SummarizationMiddleware` | `MamboSummarizationMiddleware` (extended) <br>• **Chained Summaries**: preserves prior summary content across multiple summarization rounds; uses `DEFAULT_MAMBO_CHAINED_SUMMARY_PROMPT` to instruct the model to merge historical summaries rather than overwrite, preventing cross-round information loss <br>• **CJK Token Counting**: auto-detects CJK character ratio, applying a different chars-per-token ratio than English to avoid severe token under-counting for CJK text <br>• **Latest User Message Protection**: ensures the most recent user message is never summarized away (the langchain base implementation lacks this safeguard) <br>• **Optional Backend Persistence**: evicted original messages can be written via `BackendProtocol` to `/.mambo/conversation_history/{thread_id}.md` |
...
| Sub-agent Types | `SubAgent` / `CompiledSubAgent` / `AsyncSubAgent` | `SubAgent` / `CompiledSubAgent` |
