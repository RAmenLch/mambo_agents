...
MultimodalDescriber = Callable[[VirtualPath, str, str], str | None]
"""Callback that substitutes text for a multimodal (non-text) file read.

Used to give text-only models a way to handle images, video, audio and
documents.  When a configured ``multimodal_describer`` is present, reading a
multimodal file replaces the raw base64 content block with the returned
text instead — either a natural-language description (generated by a
multimodal model) or an explicit rejection message.

Args:
    file_path: The virtual path of the file being read.
    base64_content: The base64-encoded file bytes.
    mime_type: The IANA media type (e.g. ``"image/png"``, ``"video/mp4"``).

Returns:
    A text to substitute for the multimodal content block, or ``None`` to
    leave the multimodal result unchanged (pass through).
"""
...
class BackendProtocol(abc.ABC):
    ...
    def _maybe_describe(self, result: ReadResult, file_path: VirtualPath) -> ReadResult:
        """Replace a multimodal file's base64 content with text.

        When a ``multimodal_describer`` is configured and *result* is an
        intact multimodal file (image, video, audio, document), the raw
        base64 content block is replaced with the describer's text — either
        a natural-language description or an explicit rejection message —
        so text-only models are never handed a multimodal content block.

        On any describer failure (or when the describer returns ``None`` to
        opt out) the original multimodal result is returned unchanged,
        preserving the default behaviour for multimodal models.
        """
        if (
            self._multimodal_describer is None
            or not result.is_multimodal
            or result.content is None
        ):
            return result
        try:
            desc = self._multimodal_describer(file_path, result.content, result.mime_type)
        except Exception:
            return result
        if desc is None:
            return result
